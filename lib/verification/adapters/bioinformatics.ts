...
const VALID_CLAIM_TYPES = ["sequence_analysis", "alignment", "pipeline_validation"] as const;
  ...
  async verify(taskResult, taskMetadata): Promise<VerificationResult> {
    ...
    try {
      ...
      switch (claimType) {
        ...
        default:
          return failResult(DOMAIN, [
            `Unsupported bioinformatics claim type: '${claimType}'. Valid types: ${VALID_CLAIM_TYPES.join(", ")}`,
          ]);
      }
