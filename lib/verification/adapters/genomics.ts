...
const VALID_CLAIM_TYPES = ["variant_annotation", "gene_expression", "gwas_association"] as const;
  ...
  async verify(taskResult, taskMetadata): Promise<VerificationResult> {
    ...
    try {
      ...
      switch (claimType) {
        ...
        default:
          return failResult(DOMAIN, [
            `Unsupported genomics claim type: '${claimType}'. Valid types: ${VALID_CLAIM_TYPES.join(", ")}`,
          ]);
      }
