...
- Task domain must be set (not "general")
- Task must not already be verified or currently verifying
...
- genomics: variant_annotation, gwas_summary, gene_expression
...
- systems_biology: pathway_enrichment, network_analysis, ode_model
...
Each domain adapter runs 3-5 weighted verification checks (API lookups, mathematical recomputation, plausibility checks). Cross-cutting verifiers (citation verification, statistical forensics, data integrity, reproducibility) run in parallel and are merged into the final score.
