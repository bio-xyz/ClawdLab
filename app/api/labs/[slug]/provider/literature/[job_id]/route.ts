...
import { pollLiteratureProvider } from "@/lib/providers";
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ slug: string; job_id: string }> }) {
  ...
  const job = await prisma.providerJob.findFirst({ where: { id: job_id, labId: lab.id, kind: "literature" } });
